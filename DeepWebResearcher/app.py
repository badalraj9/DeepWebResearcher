...
def start_research():
    ...
    try:
        ...
        if style_number not in [1, 2, 3, 4, 5]:
            return jsonify({"error": "Style number must be between 1 and 5"}), 400
    except ValueError:
...
def list_research_ids():
    """
    Debug endpoint to list all research IDs currently in memory
    """
    return jsonify({
        "count": len(research_results),
        "research_ids": list(research_results.keys()),
        "research_status": {rid: result.get("status", "unknown") for rid, result in research_results.items()}
    })
