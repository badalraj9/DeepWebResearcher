...
const mapContentStyleToCategory = (contentStyle: string): string => {
  switch (contentStyle?.toLowerCase()) {
    ...
    case 'latex detailed report':
      return 'latex-detailed-report';
    case 'latex executive summary':
      return 'latex-executive-summary';
    default:
      return 'blog-style';
  }
};
...
const ChatDemo = () => {
    ...
    const startPolling = async () => {
      try {
        ...
        try {
          ...
          // Check if it's a 404 error (research not found)
          if (error instanceof Error && error.message.includes('Research ID not found')) {
            console.log('Research ID not found during initial check, redirecting to home');
            ...
            setTimeout(() => {
              ...
              toast({
                ...
                description: "The research you're looking for could not be found. It may have been deleted or the server was restarted.",
                variant: "destructive"
              });
            ...
            // Don't continue with polling
            return;
          }
          
          // For other errors, continue with polling
        }
        ...
        pollInterval = setInterval(async () => {
          try {
            ...
            // Check if it's a 404 error (research not found)
            if (error instanceof Error && error.message.includes('Research ID not found')) {
              console.log('Research ID not found, redirecting to home');
  ...
  const handleEditorSearch = (query: string, category: string, additionalInfo: string) => {
    ...
    const styleNumber = 
      ...
      category === 'detailed-report' ? 2 :
      category === 'latex-detailed-report' ? 4 :
      category === 'latex-executive-summary' ? 5 : 3;
    ...
    <div className="app-container">
      <div className="flex flex-col w-full h-screen bg-gray-950 text-gray-100">
        <div className="app-container relative min-h-screen flex flex-col">
          {/* Navigation Menu - Handles both mobile and desktop */}
          <NavMenu 
