...
const EditorPage: React.FC<EditorPageProps> = ({
  ...
  // Helper to detect if content is LaTeX
  const isLatexContent = (content: string): boolean => {
    // Check if this is a research result with LaTeX style
    if (selectedItem?.category === 'latex-detailed-report') {
      return true;
    }
    
    // Check for LaTeX-style structure in content
    const latexPatterns = [
      /# Title:.*- A Comprehensive Analysis/i,
      /## Abstract/i,
      /## \d+\. Introduction/i,
      /## \d+\. Methodology/i,
      /## \d+\. Results and Findings/i,
      /## \d+\. Discussion/i,
      /## \d+\. Conclusion and Recommendations/i,
      /## References/i
    ];
  ...
  // Helper to detect if content is LaTeX executive summary
  const isLatexExecutiveSummaryContent = (content: string): boolean => {
    // Check if this is a research result with LaTeX executive summary style
    if (selectedItem?.category === 'latex-executive-summary') {
      return true;
    }
    
    // Check for LaTeX executive summary structure in content
    ...
    return executiveSummaryPatterns.some(pattern => pattern.test(content));
  };
          ...
          <div className="editor-container p-4">
            ...
            {selectedItem && (
              <TextEditor
                ...
                isLatexReport={isLatexContent(selectedItem.content)}
                isLatexExecutiveSummary={isLatexExecutiveSummaryContent(selectedItem.content)}
              />
            )}
          </div>
        </>
